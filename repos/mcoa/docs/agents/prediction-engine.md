...
- **LLM Backend**: extends existing RS tools with `explain_forecast` (which model dominated, feature importance) and `prediction_health` (model accuracy, training status, ensemble weights).
